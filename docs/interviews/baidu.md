...
> 结构：姓名 + 学校/专业 + 核心项目（一句话） + 技术栈 + 求职意向
> 控制在 1.5-2 分钟，结尾引导面试官问项目
...
| 维度 | GPT-4.5 | Claude Opus |
|------|---------|-------------|
| 厂商 | OpenAI | Anthropic |
| 上下文窗口 | 128K | 200K |
| 代码能力 | 强 | 强（Claude Code 专项优化） |
| 长文本理解 | 良好 | 更优（更大窗口） |
| 安全性/对齐 | 较好 | 更保守（Constitutional AI） |
| 价格 | 较贵 | 较贵 |
| 适用场景 | 通用、代码、多模态 | 长文档分析、复杂推理、写作 |
...
- Claude Code 直接读取文件系统，不需要预先建向量索引，所以**不走 RAG**
